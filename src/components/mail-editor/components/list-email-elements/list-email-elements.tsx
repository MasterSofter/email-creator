...
export default function ListEmailElements(props : { mailData : MailData, setMailData : React.Dispatch<React.SetStateAction<MailData>>}): React.JSX.Element {
    ...
    const addTab = () => {
      props.mailData.elements.push(new EmptyElement());
        ...
        <Accordion
            ...
            >
                { props.mailData.elements.map((element , index) => (
                        <Item key={ element.ElementKey } index={index} element={ element } removeTab={removeTab} mailData={props.mailData} setMailData={props.setMailData}/>
                )) }
                <Button
